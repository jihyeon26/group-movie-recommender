"""Run the 500-disjoint-pair Popularity, LightGCN, and ItemKNN comparison."""
...
from group_movie_recommender.pipelines.scaled_pair_experiment import (
    run_scaled_pair_exploratory_test,
    run_scaled_pair_selection,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=PROJECT_ROOT / "outputs/scaled_pair_experiment",
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs/scaled_pair_experiment.json",
    )
    parser.add_argument(
        "--preprocessing",
        type=Path,
